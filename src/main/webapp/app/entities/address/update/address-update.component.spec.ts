...
describe('Address Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call ownership query and add missing value', () => {
      ...
      const ownership: IOwnership = { id: 28016 };
      address.ownership = ownership;

      const ownershipCollection: IOwnership[] = [{ id: 14623 }];
      jest.spyOn(ownershipService, 'query').mockReturnValue(of(new HttpResponse({ body: ownershipCollection })));
      const expectedCollection: IOwnership[] = [ownership, ...ownershipCollection];
      ...
      expect(ownershipService.addOwnershipToCollectionIfMissing).toHaveBeenCalledWith(ownershipCollection, ownership);
      expect(comp.ownershipsCollection).toEqual(expectedCollection);
    });

    it('Should call country query and add missing value', () => {
      const address: IAddress = { id: 456 };
      const country: ICountry = { id: 14013 };
      address.country = country;

      const countryCollection: ICountry[] = [{ id: 11608 }];
    ...
    it('Should update editForm', () => {
      const address: IAddress = { id: 456 };
      const ownership: IOwnership = { id: 22278 };
      address.ownership = ownership;
      const country: ICountry = { id: 19021 };
      address.country = country;

      activatedRoute.data = of({ address });
      comp.ngOnInit();

      expect(comp.ownershipsCollection).toContain(ownership);
