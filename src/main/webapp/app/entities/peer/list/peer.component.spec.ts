...
describe('Peer Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'peer', component: PeerComponent }]), HttpClientTestingModule, PeerComponent],
      providers: [
        {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.peers?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to peerService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getPeerIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getPeerIdentifier).toHaveBeenCalledWith(entity);
